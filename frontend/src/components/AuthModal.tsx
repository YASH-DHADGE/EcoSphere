...
//above is the old code for AuthModal.tsx which will be used when backend is ready
//and now below is the temporary code for AuthModal.tsx which is just harcoded login/signup page
...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // --------------------------------------------------
    // HARD-CODED TEST LOGIN (NO API CALL)
    // --------------------------------------------------
    ...
    if (isLogin) {
      ...
      if (formData.email === testEmail && formData.password === testPassword) {
        ...
        localStorage.setItem("user", JSON.stringify({ name: "Test User", email: testEmail }));
        navigate("/dashboard");
        return;
      } else {
      ...
      // SIGNUP MODE — accept anything
      const newUser = {
        name: formData.name,
        email: formData.email,
      };

      localStorage.setItem("token", "TEST_SIGNUP_TOKEN_456");
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border 
                           ...
                           focus:outline-none focus:ring-primary-green 
          ...
          <button
            ...
            className="w-full py-2 px-4 border border-transparent rounded-md shadow-sm 
                       text-sm font-medium text-white bg-primary-green hover:bg-green-500 
